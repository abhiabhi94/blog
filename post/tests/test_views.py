...
class TestImageLicensePage(TestBaseView):
    def test_content_policy_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'post/image_license.html')
...
class TestPrivacyPolicyPage(TestBaseView):
    def test_privacy_policy_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'post/privacy_policy.html')


class TestRecommendedArticles(TestPostBase, TestAJAXView):

    def get_url(self):
        return reverse('post:recommended-posts')
...
class TestLatestArticles(TestPostBase, TestAJAXView):
    ...
    def get_url(self):
        return reverse('post:latest-posts')
...
class TestBookmarkPosts(TestPostBase, TestAJAXView):
    def get_url(self):
        return reverse('post:bookmark-post')
...
class TestUserPostView(TestPostBase, TestBaseView):
    ...
    def get_url(self, username=None, tab=None):
        ...
        if not tab:
            tab = 'draft'
        return reverse('post:my-posts', kwargs={'username': username, 'tab': tab})
...
class TestAuthorPostView(TestPostBase, TestBaseView):
    def get_url(self, username=None):
        if not username:
            username = self.user.username
        return reverse('post:author-posts', kwargs={'username': username})
    ...
    def test_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'post/author_posts.html')
        result = response.context
...
class TestUserBookmarkView(TestPostBase, TestBaseView):
    ...
    def get_url(self, username=None):
        if not username:
            username = self.user.username
        return reverse('post:user-bookmarks', kwargs={'username': username})
...
class TestPostDetailView(TestPostBase, TestBaseView):
    def get_url(self, post=None):
        ...
        return reverse('post:post-detail', kwargs={
            'slug': post.slug,
...
class TestDraftPostUpdateView(TestPostBase, TestBaseView):
    ...
    def get_url(self, post=None):
        ...
        return reverse('post:draft-post-update', kwargs={'slug': post.slug})
